...
def chaboche_samples(number_training_points):
    """
    Generate samples for the Chaboche Model

    # ? create a genertor instead for efficiency

    Parameters
    ----------
    number_training_points : int
                             Number of samples 
...
def chaboche_uniform_isoprob_transform(any_X):
    """
    Apply isoprobabilistic transform to samples
...
def chaboche_uniform_isoprob_monotonic_transform(any_X):
    """
    Apply isoprobabilistic transform to samples for monotonic loading
...
class ChabocheModel():
    ...
    # TODO : 2D problem (partly implemented)
    ...
    loading_type : str
                   'monotonic' or 'cyclic'
    max_applied_strain : float
                         maximum strain applied in X direction
    time_to_max_strain : float
    ...
    def equivalent_stress(self, sigma, backstress):
        ...
        '''
        1D loading
        effective stress for a given stress state
...
def generate_model_evaluations(model, t_min, t_max, delta_t, method, SampleSpace, parallel_computation=True):
    ...
    func_evaluations = np.zeros((n_t, number_of_realizations))
